...
- Recurrence Relation 递推关系：Hanoi 塔问题 $H_1=1,H2=3,H_n=2H_{n-1}+1$
- 线性齐次递推关系（LHRR）：$a_n=c_1a_{n-1}+c_2a_{n-2}+\cdots+c_ka_{n-k}$，有通项公式
- 特征根：LHRR 变换为 $r^k-c_1r^{k-1}-c_2r^{k-2}-\cdots-c_k=0$，可解得 $k$ 个特征根，再带回前 $k$ 个初始值 $a_n=c_1r_1^n+c_2r_2^n+\cdots+c_kr_k^n$，解出 $c_1,\cdots,c_k$
- 线性非齐次递推关系（LNRR）：$a_n=c_1a_{n-1}+c_2a_{n-2}+\cdots+c_ka_{n-k}+F(n)$ ；则 $a_n=$ 齐次通解 $+$ 特解
- LNRR 特解：若 $F(n)=P(n)s^n$，其中 $P(n)$ 为 $n$ 的 $l$ 阶多项式，$s$ 为特征根之一，其重数为 $m$，则特解 $x_n=(p_ln^l+\cdots+p_1n+p_0)s^nn^m$
- 